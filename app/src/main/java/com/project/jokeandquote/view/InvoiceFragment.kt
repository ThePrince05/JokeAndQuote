...
import com.project.jokeandquote.databinding.FragmentInvoiceBinding
import com.project.jokeandquote.viewmodel.InvoiceViewModel
import kotlinx.coroutines.launch
import java.text.SimpleDateFormat
import java.util.Date
import java.util.Locale

class InvoiceFragment : FadeThroughFragment(R.layout.fragment_invoice) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.companyName.observe(viewLifecycleOwner) { text ->
            fromUser = false
            if (binding.companyNameInput.text.toString() != text) {
                binding.companyNameInput.setText(text)
            }
        }
        viewModel.companyAddress.observe(viewLifecycleOwner) { text ->
            fromUser = false
            if (binding.companyAddressInput.text.toString() != text) {
                binding.companyAddressInput.setText(text)
            }
        }
        ...
        viewModel.eventAddress.observe(viewLifecycleOwner) { text ->
            fromUser = false
            if (binding.eventAddressInput.text.toString() != text) {
                binding.eventAddressInput.setText(text)
            }
        }
        ...
        binding.companyNameInput.doAfterTextChanged {
            if(fromUser) {
                viewModel.companyName.value = it.toString()
            }
            fromUser = true
        }

        binding.companyAddressInput.doAfterTextChanged {
            if(fromUser) {
                viewModel.companyAddress.value = it.toString()
            }
            fromUser = true
        }

        binding.eventAddressInput.doAfterTextChanged {
            if(fromUser) {
                viewModel.eventAddress.value = it.toString()
            }
            fromUser = true
        }
        ...
        binding.btnGenerate.setOnClickListener {
            lifecycleScope.launch {
                if (validateFields()) {
                    ...
                    try {
                        val success = viewModel.generateInvoicePDF(requireContext())
    ...
    private fun capitalizeFields() {
        ...
        viewModel.companyName.value = viewModel.companyName.value?.capitalizeWords()
        viewModel.companyAddress.value = viewModel.companyAddress.value?.capitalizeWords()
        viewModel.eventName.value = viewModel.eventName.value?.capitalizeWords()
        viewModel.eventAddress.value = viewModel.eventAddress.value?.capitalizeWords()
    }
    ...
    private fun validateFields(): Boolean {
        ...
        if (viewModel.companyName.value.isNullOrBlank()) {
            binding.companyNameInput.error = "Company name is required"
            isValid = false
        }
        if (viewModel.companyAddress.value.isNullOrBlank()) {
            binding.companyAddressInput.error = "Company address is required"
            isValid = false
        }
        ...
        if (viewModel.eventAddress.value.isNullOrBlank()) {
            binding.eventAddressInput.error = "Event Address is required"
            isValid = false
        }
